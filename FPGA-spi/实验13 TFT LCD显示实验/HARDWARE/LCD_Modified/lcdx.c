...
void LCD_ShowCoordinateSystem(u16 xbasepoint,u16 ybasepoint,u16 xstep,u16 ystep,u16 xsteplength,u16 ysteplength,u16 xbasevalue,u16 ybasevalue,u16 xstepvalue,u16 ystepvalue)
{
	int i;
	LCD_DrawLine(xbasepoint, ybasepoint, xbasepoint+xsteplength*xstep, ybasepoint); //axis X
	LCD_DrawLine(xbasepoint, ybasepoint, xbasepoint, ybasepoint-ysteplength*ystep); //axis Y
	LCD_DrawLine(xbasepoint+xsteplength*xstep-10, ybasepoint+7, xbasepoint+xsteplength*xstep, ybasepoint); //arrow X
	LCD_DrawLine(xbasepoint+xsteplength*xstep-10, ybasepoint-7, xbasepoint+xsteplength*xstep, ybasepoint); //arrow X
	LCD_DrawLine(xbasepoint+7, ybasepoint-ysteplength*ystep+10, xbasepoint, ybasepoint-ysteplength*ystep); //arrow Y
	LCD_DrawLine(xbasepoint-7, ybasepoint-ysteplength*ystep+10, xbasepoint, ybasepoint-ysteplength*ystep); //arrow Y
	LCD_ShowxNum(xbasepoint-10,ybasepoint+5,0,1,24,1);  //写零点
	LCD_ShowxString(xbasepoint+xsteplength*xstep, ybasepoint+5,24,24,24,(u8*)"X",1); //写x
  LCD_ShowxString(xbasepoint-20, ybasepoint-ysteplength*ystep-10,24,24,24,(u8*)"y",1); //写y
	
	for(i=1;i<xstep;i++)
	{
		LCD_DrawLine(xbasepoint+xsteplength*i,ybasepoint,xbasepoint+xsteplength*i,ybasepoint-5);//Horizontal Markers
    LCD_ShowxNum(xbasepoint+xsteplength*i-20,ybasepoint+5,xbasevalue+xstepvalue*i,3,24,1);
	}
	for(i=1;i<ystep;i++)
	{
		LCD_DrawLine(xbasepoint,ybasepoint-ysteplength*i,xbasepoint+5,ybasepoint-ysteplength*i);//Vertical Markers
    LCD_ShowxNum(xbasepoint-40,ybasepoint-ysteplength*i-10,ybasevalue+ystepvalue*i,3,24,1);
	}
}
...
void Display_Color(void)
{
	...
	for(i=0;i<256;i++)
	{
		POINT_COLOR=RGB(i,0,255);
		LCD_DrawLine(i,0,i,250);
	}
	for(i=256;i<512;i++)
	{
		POINT_COLOR=RGB(0,i-256,255);
		LCD_DrawLine(i,0,i,250);
	}
	for(i=512;i<768;i++)
	{
		POINT_COLOR=RGB(255,0,i-512);
		LCD_DrawLine(i,0,i,500);
	}
	for(i=0;i<256;i++)
	{
		POINT_COLOR=RGB(i,255,0);
		LCD_DrawLine(i,251,i,500);
	}
	for(i=256;i<512;i++)
	{
		POINT_COLOR=RGB(255,i-256,0);
		LCD_DrawLine(i,251,i,500);
	}
	for(i=512;i<768;i++)
	{
		POINT_COLOR=RGB(0,255,i-512);
		LCD_DrawLine(i,251,i,500);
	}}
